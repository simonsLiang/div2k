...
class IMDModule(nn.Module):
    def __init__(self, in_channels=24, distillation_rate=0.25):
        super(IMDModule, self).__init__()
        self.distilled_channels = int(in_channels * distillation_rate)
        self.remaining_channels = int(in_channels - self.distilled_channels)
        self.c1 = swinir.RSTB(dim=in_channels)

        self.c2 = swinir.RSTB(dim=self.remaining_channels)
        self.cc2 = conv_layer(self.remaining_channels, in_channels, 1)

        self.c3 = swinir.RSTB(dim=self.remaining_channels)
        self.cc3 = conv_layer(self.remaining_channels, in_channels, 1)

        self.c4 = swinir.RSTB(dim=self.remaining_channels)
        self.cc4 = conv_layer(self.remaining_channels,self.distilled_channels,1)
    ...
    def forward(self, input):
        ...
        out_c1 = self.act(self.c1(input,input_resolution))
        distilled_c1, remaining_c1 = torch.split(out_c1, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c2 = self.act(self.cc2(self.c2(remaining_c1,input_resolution)))
        distilled_c2, remaining_c2 = torch.split(out_c2, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c3 = self.act(self.cc3(self.c3(remaining_c2,input_resolution)))
        distilled_c3, remaining_c3 = torch.split(out_c3, (self.distilled_channels, self.remaining_channels), dim=1)
        out_c4 = self.cc4(self.c4(remaining_c3,input_resolution))
